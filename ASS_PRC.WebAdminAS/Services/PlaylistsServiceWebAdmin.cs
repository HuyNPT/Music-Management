...
namespace ASS_PRC.WebAdminAS.Services
{
    public class PlaylistsServiceWebAdmin : IPlaylistsServiceWebAdmin
    {
        ...
        public async Task<bool> PostPlaylist(string PlaylistName, string ImageUrl, List<Guid> Category, string jwt)
        {
            var client = _httpClientFactory.CreateClient();
            client.BaseAddress = new Uri(_config["BaseUrl"]);
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jwt);
        ...
        public async Task<string> PutPlayListWebAdmin(string PlaylistId, string Name, string ImageUrl, List<Guid> Category, string jwt)
        {
            var client = _httpClientFactory.CreateClient();
            client.BaseAddress = new Uri(_config["BaseUrl"]);
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jwt);
