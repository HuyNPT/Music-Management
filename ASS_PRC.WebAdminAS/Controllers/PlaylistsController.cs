...
namespace ASS_PRC.WebAdminAS.Controllers
{
    ...
    public class PlaylistsController : Controller
    {
        ...
        public async Task<IActionResult> Playlist(string page)
        {
            var sessions = HttpContext.Session.GetString("Token");
            int pageCurrent = 1;
            try
        ...
        public async Task<IActionResult> PlayPlaylist(string id)
        {
            var playlistSelected = await _playlistsServiceWebAdmin.GetPlaylistById(id);
            var listMedia = await _mediaServiceWebAdmin.GetMedia(id, 0);
            Playlist x = new Playlist()
        ...
        public async Task<IActionResult> UpdatePlaylist(int page, PlaylistResponse model)
        {
            ...
            var rs = await _playlistsServiceWebAdmin.PutPlayListWebAdmin(model.Id.ToString(), model.PlaylistName, model.ImageUrl, cate, sessions);
