...
namespace ASS_PRC.Api.Controllers
{
    ...
    public class MediaController : ControllerBase
    {
        ...
        public async Task<IActionResult> PostNediaAdmin([FromBody] PostMediaRequest request)
        {
            ...
            var rs = await _mediaService.PostMedia(request.MediaName, request.Url, request.ImgUrl, request.Author, request.Singer, request.Type, request.Category,UserID, PlaylistID, request.FileName);
            if(rs){
                return Ok();
            }
            else
        ...
        {          
            var rs = await _mediaService.DeleteMediaWebAdmin(MediaID);            
            if (!rs)
            {
                return BadRequest();
            }
        ...
        {
            Guid UserID = new Guid(User.FindFirst(ClaimTypes.NameIdentifier)?.Value);
            var rs = await _mediaService.PutMedia(rq.MediaID, rq.MediaName, rq.Url, rq.ImgUrl, rq.Author, rq.Singer, rq.Type, rq.Category, UserID,rq.FileName);
            if (rs == "Unauthorized")
            {
                return Unauthorized();
            }
